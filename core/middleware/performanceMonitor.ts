 ...
 * - Recent requests: ~6.4KB (100 entries × 64 bytes each)
...
export class PerformanceMonitor {
  ...
  private formatUptime(ms: number): string {
    ...
    if (days > 0) {
      ...
      // Example: 27 hours → 27 % 24 = 3 hours (1 day + 3 hours)
   ...
   * Formula: i = floor(log₁₀₂₄(bytes))
   *
   * WHY LOGARITHMS?
   * Each unit is 1024× the previous:
   * - 1 KB = 1024 B
   * - 1 MB = 1024 KB = 1024² B
   * - 1 GB = 1024 MB = 1024³ B
   *
   ...
   *
   * EXAMPLES:
   * - 1024 bytes     → log₁₀₂₄(1024) = 1       → sizes[1] = "KB" → "1 KB"
   * - 1048576 bytes  → log₁₀₂₄(1048576) = 2    → sizes[2] = "MB" → "1 MB"
   * - 42567890 bytes → log₁₀₂₄(42567890) = 2.5 → sizes[2] = "MB" → "40.6 MB"
   *
   * @private
  ...
  private formatBytes(bytes: number): string {
    ...
    // Example: i=2 → 1024² = 1048576 (bytes per MB)
    const value = bytes / Math.pow(1024, i);

    // Round to 2 decimal places
    // Multiply by 100, round, divide by 100
    // Example: 40.6234 → 4062.34 → 4062 → 40.62
  ...
  private getTopEndpoints(limit: number = 5) {
    return Array.from(this.endpoints.entries())
      ...
      // b.count - a.count → descending order (largest first)
      .sort(([, a], [, b]) => b.count - a.count)
      // Take only the top N entries
      // slice(0, 5) → elements at indices 0, 1, 2, 3, 4
      .slice(0, limit)
  ...
  private startPeriodicCleanup(): void {
    ...
    setInterval(() => {
      ...
      // req.timestamp > fiveMinutesAgo → keep
      // req.timestamp <= fiveMinutesAgo → remove
      this.recentRequests = this.recentRequests.filter(
        (req) => req.timestamp > fiveMinutesAgo,
      );

      // Optional: Log cleanup for monitoring
      // console.log(`🧹 Cleanup: ${this.recentRequests.length} requests retained`);
    }, 5 * 60 * 1000);
  }
  ...
  getPerformanceInsights() {
    ...
    // Health calculation:
    // 1. If no insights → excellent (all metrics within thresholds)
    // 2. If any error insights → poor (critical issues present)
    // 3. If only warning insights → good (optimization recommended)
...
function generateRequestId(): string {
  ...
  // Math.random() → 0.7234... (random decimal 0-1)
  // .toString(36) → "0.q3f2k9..." (base-36: 0-9, a-z)
  // .substring(2, 8) → "q3f2k9" (6 characters)
  const randomPart = Math.random().toString(36).substring(2, 8);

  // Current timestamp in base-36 (shorter representation)
  // Date.now() → 1697900000000 (Unix timestamp in ms)
  // .toString(36) → "1n7s9p" (base-36 representation)
 ...
 * EXECUTION FLOW:
 * ```
 * Request → Middleware → next() → Handler
 *                    ↓               ↓
 * Response ← Middleware ← ← ← ← ← ← ←
 * ```
 *
